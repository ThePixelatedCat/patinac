use cranelift::{
    codegen::ir::{ExtFuncData, ExternalName, FuncRef, KnownSymbol, LibCall, UserExternalNameRef},
...
impl<'ctx> Codegen<'_, '_> {
    pub(crate) fn printf(&self) -> ExtFuncData {
        // ExtFuncData {
        //     name: ExternalName::User(UserExternalNameRef::)
        //     patchable: false
        // }
        // if let Some(FuncOrDataId::Func(func)) = self.module.declarations().get_name("printf") {
        //     return func;
        // }
        let ty = self.ctx.i32_type().fn_type(&[self.ptr_ty().into()], true);
        self.module
            .declare_function("printf", Linkage::Import, ty)
            .unwrap();
    }
