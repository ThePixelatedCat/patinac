...
impl Codegen<'_, '_> {
    pub(crate) fn drop_func_sig(&self) -> Signature {
        Signature {
    ...
    pub(crate) fn copy_func_sig(&self) -> Signature {
        let ptr = AbiParam::new(self.ptr_ty());
        Signature {
            call_conv: CallConv::Fast,
            params: vec![ptr, ptr],
            returns: vec![],
    ...
    pub(crate) fn equals_func_sig(&self) -> Signature {
        let ptr = AbiParam::new(self.ptr_ty());
        Signature {
            call_conv: CallConv::Fast,
            params: vec![ptr, ptr],
    ...
    pub(crate) fn tuple_drop(&self, ty: &Ty, inner_tys: &[Ty]) -> FuncId {
        self.fields_drop(ty, inner_tys)
    }

    pub(crate) fn tuple_copy(&self, ty: &Ty, inner_tys: &[Ty]) -> FuncId {
        self.fields_copy(ty, inner_tys)
    }

    pub(crate) fn tuple_equals(&self, ty: &Ty, inner_tys: &[Ty]) -> FuncId {
        self.fields_equals(ty, inner_tys)
    }

    pub(crate) fn struct_drop(&self, id: TyId) -> FuncId {
        self.fields_drop(&Ty::Named(id), self.hir.ty_info(id).fields.tys())
    }

    pub(crate) fn struct_copy(&self, id: TyId) -> FuncId {
        self.fields_copy(&Ty::Named(id), self.hir.ty_info(id).fields.tys())
    }

    pub(crate) fn struct_equals(&self, id: TyId) -> FuncId {
    ...
    fn fields_drop<'fields>(
        ...
        let func = self
            .module
            .declare_function(&func_name, Linkage::Local, &self.drop_func_sig())
            .unwrap();
        self.builder
    ...
    fn fields_copy<'fields>(
        ...
        let func =
            self.module
                .add_function(&func_name, self.copy_func_sig(), Some(Linkage::Private));
        self.builder
            .position_at_end(self.ctx.append_basic_block(func, "entry"));
        let lowered_ty = self.lower_ty(ty);
    ...
    fn fields_equals<'fields>(
        ...
        let func =
            self.module
                .add_function(&func_name, self.equals_func_sig(), Some(Linkage::Private));
        self.builder
            .position_at_end(self.ctx.append_basic_block(func, "entry"));
        let lowered_ty = self.lower_ty(ty);
    ...
    pub(crate) fn array_drop(&self, ty: &Ty, elem_ty: &Ty) -> FuncId {
        let func_name = format!("{}.drop", self.mangle_ty(ty));
        ...
        let func =
            self.module
                .add_function(&func_name, self.drop_func_sig(), Some(Linkage::Private));
    ...
    pub(crate) fn array_copy(&self, ty: &Ty) -> FuncId {
        let func_name = format!("{}.copy", self.mangle_ty(ty));
    ...
    pub(crate) fn array_equals(&self, ty: &Ty, elem_ty: &Ty) -> FuncId {
        let func_name = format!("{}.equals", self.mangle_ty(ty));
    ...
    pub(crate) fn array_init(&self, ty: &Ty, elem_ty: &Ty) -> FuncId {
    ...
    pub(crate) fn array_unique(&self, ty: &Ty, elem_ty: &Ty) -> FuncId {
    ...
    pub(crate) fn array_calc_cap(&self, ty: &Ty, elem_ty: &Ty) -> FuncId {
    ...
    pub(crate) fn array_bounds_check(&self) -> FuncId {
    ...
    pub(crate) fn closure_drop(&self) -> FuncId {
    ...
    pub(crate) fn closure_copy(&self) -> FuncId {
    ...
    pub(crate) fn closure_equals(&self) -> FuncId {
